...
const MAX_NO_FOCUS_TIME = 1100;

const MIN_SCORE = 0.40;
const NO_FOCUS_FACE_BLEND_SHAPES = [
...
const REQ_SCORE = 0.42;
const NO_FOCUS_FACE_BLEND_SHAPE = [
...
let focusStartTime = undefined;
let noFocusStartTime = undefined;
...
async function predictWebcam() {
  ...
  if (eyeFocusFunctionEnabled === true) {
    ...
    if (eyeFocusResults.faceLandmarks) {
      if (eyeFocusResults.faceBlendshapes.length !== 0)
        console.log(
          "results:",
          eyeFocusResults.faceBlendshapes[0].categories.filter(
            (shape) => shape.score > 0.1
          )
        );

      var noFocusResult =
        eyeFocusResults.faceBlendshapes.length !== 0
          ? eyeFocusResults.faceBlendshapes[0].categories.filter(
              (shape) => shape.score > MIN_SCORE && shape.categoryName.startsWith('eye')
            ).map(e => e.categoryName)
          : [];

      var noFocusSecondResult =
        eyeFocusResults.faceBlendshapes.length !== 0
          ? eyeFocusResults.faceBlendshapes[0].categories.filter(
              (shape) => shape.score > REQ_SCORE
            ).map(e => e.categoryName)
          : [];

      if (eyeFocusResults.faceLandmarks.length === 0 || noFocusResult.length > 1 || noFocusSecondResult.length !== 0) {
        var status = false;

        if (eyeFocusResults.faceLandmarks.length !== 0) {
          for (let i = 0; i < NO_FOCUS_FACE_BLEND_SHAPES.length; i++) {
            if (NO_FOCUS_FACE_BLEND_SHAPES[i].every((r) => noFocusResult.includes(r))) {
              status = true;
              break;
            }
          }
          ...
          if (!status) {
            for (let i = 0; i < NO_FOCUS_FACE_BLEND_SHAPE.length; i++) {
              if (noFocusSecondResult.includes(NO_FOCUS_FACE_BLEND_SHAPE[i][0]) && !noFocusSecondResult.includes(NO_FOCUS_FACE_BLEND_SHAPE[i][1])) {
                status = true;
                break;
              }
            }
        ...
        console.log("status:", status, "noFocusResult:", noFocusResult)

        if (status === true) {
          focusStartTime = undefined;
          if (!automaticStopped) {
            if (noFocusStartTime === undefined) {
              noFocusStartTime = Date.now();
            } else {
              var now = Date.now();

              if (now - noFocusStartTime > MAX_NO_FOCUS_TIME) {
                YTPlayerController(null, null, null, "noFocus");
                noFocusStartTime = undefined;
                automaticStopped = true;
                console.log(
                  "stoppo",
                  eyeFocusResults.faceLandmarks.length === 0,
                  noFocusResult
                );
              }
            }
      ...
      } else if (automaticStopped) {
        console.log("resetto noFocusStartTime")
        noFocusStartTime = undefined;

        if (focusStartTime === undefined) {
          focusStartTime = Date.now();
        } else {
          var now = Date.now();

          if (now - focusStartTime > MAX_NO_FOCUS_TIME) {
            YTPlayerController(null, null, null, "focus");
            focusStartTime = undefined;
            automaticStopped = false;
            console.log("avvio", eyeFocusResults.faceLandmarks.length === 0);
          }
        }
      }
    ...
    //
    //    if (currentTime - noFocusStartTime > MAX_NO_FOCUS_TIME) {
    //      noFocusStartTime = undefined;
    //      YTPlayerController(null, null, null, "noFocus")
    //    }
    //  }
    ...
    if (gestureResults) {
      if (gestureResults.gestures.length > 0) {
        if (webcamRunning) {
          ...
          //console.log(rightHandGesture, typeof(rightHandGesture))

          if (
            (rightHandGesture !== null && rightHandGesture.length !== 0) || 
            (leftHandGesture !== null && leftHandGesture.length !== 0) || 
            (totalFingerCount !== 0) 
          ) {
            focusStartTime = undefined;
            noFocusStartTime = undefined;
